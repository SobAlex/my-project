...
@section('title', 'Редактирование категории отрасли')
...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                <span
                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $industryCategory->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
    ...
    <form action="{{ route('admin.industry-categories.update', $industryCategory) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" id="name"
                                value="{{ old('name', $industryCategory->name) }}" placeholder="Введите название категории"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="slug" id="slug"
                                value="{{ old('slug', $industryCategory->slug) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center space-x-3">
                                    <input type="color" name="color" id="color"
                                        value="{{ old('color', $industryCategory->color) }}"
                                    ...
                                    <input type="text" name="color_text" id="color_text"
                                        value="{{ old('color', $industryCategory->color) }}" placeholder="#06b6d4"
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="icon" id="icon"
                                    value="{{ old('icon', $industryCategory->icon) }}"
                                    placeholder="Например: business, store, factory"
                                ...
                                @enderror
                                <p class="text-xs text-gray-500">Введите название иконки Material Icons (например: business,
                                    store, factory)</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="description" id="description" rows="8" placeholder="Краткое описание категории"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 resize-none @error('description') border-red-300 ring-2 ring-red-100 @enderror">{{ old('description', $industryCategory->description) }}</textarea>
                            @error('description')
                                <p class="flex items-center text-sm text-red-600 mt-1">
                                    <i class="material-icons text-red-500 mr-1 text-sm">error</i>
                                    {{ $message }}
                            ...
                            <div id="category-preview" class="p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <div class="flex items-center space-x-3">
                                    <div id="preview-icon"
                                        ...
                                        style="background-color: {{ old('color', $industryCategory->color) }}">
                                        <i class="material-icons"
                                            id="preview-icon-name">{{ old('icon', $industryCategory->icon) }}</i>
                                    </div>
                                    <div>
                                        <div id="preview-name" class="text-sm font-medium text-gray-900">
                                            {{ old('name', $industryCategory->name) }}</div>
                                        <div id="preview-description" class="text-xs text-gray-500">
                                            {{ old('description', $industryCategory->description) ?: 'Описание категории' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="flex items-center">
                                        ...
                                        <input type="checkbox" name="is_active" id="is_active" value="1"
                                            {{ old('is_active', $industryCategory->is_active) ? 'checked' : '' }}
                                        ...
                                        <div class="flex items-start space-x-3">
                                            ...
                                            <div class="flex-1">
                                                <p class="text-sm text-yellow-800 font-medium">
                                                    В этой категории {{ $industryCategory->cases()->count() }} кейсов
                                                </p>
                                                <p class="text-xs text-yellow-700 mt-1">
                                                    При деактивации категории все кейсы будут скрыты с сайта
                                                </p>
                                                <div class="mt-3">
                                                    <label class="flex items-center">
                                                        ...
                                                        <span class="ml-2 text-xs text-yellow-800 font-medium">
                                                            Принудительно деактивировать (скрыть все кейсы)
                                                        </span>
                                                    </label>
                                                </div>
                                            </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" name="sort_order" id="sort_order"
                                value="{{ old('sort_order', $industryCategory->sort_order) }}" min="0"
                                placeholder="0"
    ...
    <script>
        ...
        (function() {
            ...
            function updatePreview() {
                ...
                previewIconName.textContent = iconSelect.value || 'business';
            }
